...
## Findings from the first version, and their state
...
2. **Commit-history rule.** Clarified. The rules page states: "Any repositories with single
   ...
   This repository has a bootstrap commit, a foundation commit and a remediation commit with
   dated history, and future sprint work adds auditable commits as it lands. No claim is made
   that the present history is disqualified, and no history is rewritten.
...
| --- | ------------------------------------------------------------------------------------------- | ------ | ---------------------------- | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| A1  | "Use The Graph as a load-bearing part of the project" through Subgraphs, MCP, or Substreams | EVENT  | VERIFIED FROM EVENT MATERIAL | PLANNED                      | `@cas/graph-evidence` supplies live provider-backed signals that corroborate canonical incidents; query provenance logged (Sprints 1 and 5)   |
...
| A8  | Feedback submission to The Graph                                                            | EVENT  | VERIFIED FROM EVENT MATERIAL | not required                 | The prizes page imposes no feedback requirement on any Graph track. The earlier `UNVERIFIED` row is withdrawn.                                |
...
| #   | Requirement                                                                                                                                                          | Source | Requirement verification     | Delivery    | Planned evidence                                                                                                                                                                     |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| B1  | Route 1: "compose two or more of The Graph's products"                                                                                                               | EVENT  | VERIFIED FROM EVENT MATERIAL | not pursued | Not required. Substreams stays optional and never a prerequisite; Graph Market access only if an optional path needs it (D11)                                                        |
...
Attempted in Sprint 7 only if the Graph release gate passes at the end of 10 September and
the remaining time budget allows. Testnet only (`SECURITY.md`).

| #   | Requirement                                                                                                    | Source | Requirement verification     | Delivery                     | Planned evidence                                                                         |
| --- | -------------------------------------------------------------------------------------------------------------- | ------ | ---------------------------- | ---------------------------- | ---------------------------------------------------------------------------------------- |
| C1  | "Host a live x402-gated service on Hedera testnet or mainnet"                                                  | EVENT  | VERIFIED FROM EVENT MATERIAL | PLANNED                      | `@cas/feed-api` behind an x402 gate on testnet; depends on D8 hosting and Sprint 7       |
...
| C5  | Hedera x402 integration only after the Graph release candidate passes                                          | PLAN   | UNVERIFIED (plan-level)      | PLANNED                      | Sprint 7 conditional entry in `SPRINT_BOARD.md`                                          |

## D. Bazantic: API and recipe tracks

Attempted in Sprint 7 under the same condition as section C. The plan does not fix which
...
| D4  | Screen recording demonstrating the recipe                                                     | EVENT  | VERIFIED FROM EVENT MATERIAL | PLANNED  | Sprint 7 or 9                                                             |
...
| E3  | "Any repositories with single commits of large files without proper history will be default assumed to be unqualified unless proven otherwise."                                                      | EVENT  | VERIFIED FROM EVENT MATERIAL | PLANNED                      | Finding 2 above. The history has three dated commits after Sprint 0 and grows with each sprint. No claim of disqualification, no rewrite |
...
| F4  | Feedback document for The Graph         | EVENT  | VERIFIED FROM EVENT MATERIAL | not required                 | As A8                                                                                          |
...
| I2  | Project Check-in #1: 7 September, 11:59 PM       | EVENT  | VERIFIED FROM EVENT MATERIAL (transcribed by the project owner; fetch HTTP 500) | PLANNED  | Sprint 2 exit gate                                                      |
| I3  | Project Check-in #2: 10 September, 11:59 PM      | EVENT  | VERIFIED FROM EVENT MATERIAL (transcribed by the project owner; fetch HTTP 500) | PLANNED  | Sprint 5 exit gate                                                      |
