...
| --- | ---------------------------------------------------------------------------------------------- | ------ | ---------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1  | "Use The Graph as a load-bearing part of the project" through Subgraphs, MCP, or Substreams    | EVENT  | VERIFIED FROM EVENT MATERIAL | IMPLEMENTED, PENDING AUDIT                       | The live subgraph query path exists (Sprint 1, `@cas/graph-evidence`). Sprint 5 adds the load-bearing use: Graph signals correlate with canonical incidents on recorded chain and protocol identity, resolve into four explicit evidence states, and feed the chain half of the anomaly feed (`SPRINT-5-REPORT.md`, D25). Sprint 5 has not been audited |
...
| A9  | Reusable MCP tooling with `SKILL.md` and clean installation, complete at the 10 September gate | PLAN   | UNVERIFIED (decision D16)    | IMPLEMENTED ON THE PARALLEL TRACK, PENDING AUDIT | `@cas/mcp-server` built on `parallel/s6-mcp-tooling` (decision D28): four read-only tools over local stdio, `SKILL.md`, one-command frozen-lockfile installation verified from a fresh clone (`docs/MCP-TOOLING-TRACK-REPORT.md`). Not merged; pending its own Codex Desktop audit; final integration begins from the Codex-accepted Sprint 5 revision  |
...
| B4  | Sprint 1 proof gate: one common query and data model; at least five relevant protocols or entities; live provider-backed results; Ethereum mandatory; Base secondary | PLAN   | UNVERIFIED (decision D11)    | IMPLEMENTED | Final corrected gate passed 6 Sept 2026 00:19 EDT (D19, D18): five distinct canonical identities (chain plus provider slug), subgraph IDs and deployment IDs on Ethereum at block 25915866, each validated against the registry's declared expectations including the expected provider slug; Base PASS/KEEP with both configured targets verified at block 50939508. The first verifier's result (D17) is superseded |
...
| I2  | Project Check-in #1: 7 September, 11:59 PM       | EVENT  | VERIFIED FROM EVENT MATERIAL (transcribed by the project owner; fetch HTTP 500) | DELIVERED | Sprint 3 exit gate. Submitted; the project owner confirmed submission on 8 September 2026. No submission time or receipt identifier is recorded, because none was supplied. `docs/CHECKIN-1-DRAFT.md` holds the text as sent |
| I3  | Project Check-in #2: 10 September, 11:59 PM      | EVENT  | VERIFIED FROM EVENT MATERIAL (transcribed by the project owner; fetch HTTP 500) | PLANNED   | Sprint 6 exit gate, with the Graph release gate. `docs/CHECKIN-2-DRAFT.md` is drafted and **not submitted**; its cutoff time is unconfirmed and must be read from the portal                                                 |
