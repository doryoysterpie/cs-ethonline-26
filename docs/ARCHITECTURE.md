...
Built on the parallel branch `parallel/s6-mcp-tooling` (decision D28) and **pending an
independent audit**. A read-only MCP server over local stdio with four static tools.

| Module                        | Responsibility                                                                                                                                                                                                                                                                          |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/definitions.ts`          | The frozen catalogue: names, titles, descriptions, annotations, strict zod input and output schemas, the canonical JSON Schema rendering and its pinned SHA-256, checked at start-up.                                                                                                   |
| `src/schemas/input.ts`        | Four flat strict input schemas of UUIDs, enumerations, bounded integers and UTC instants, with the cross-field rules for anomaly modes and draft periods.                                                                                                                               |
| `src/schemas/output.ts`       | Strict public-safe output contracts: run provenance, incident summaries, sources and associations as quoted evidence, stored and live anomaly entries, draft previews.                                                                                                                  |
| `src/validation.ts`           | The hardened argument boundary: plain object, no symbol key, no accessor, allowlisted names, primitives only, bounded strings without control or separator characters, then the schema. Rejections never echo a value.                                                                  |
| `src/safety/text.ts`          | `quoteEvidence`: retrieved text as a single-line, escaped, bounded display copy marked `untrusted_quoted_evidence`; angle brackets escaped so a tag cannot read as markup.                                                                                                              |
| `src/safety/redact.ts`        | One redactor for the connection string, its password in both forms, the Graph key, bearer tokens and PostgreSQL URL shapes, applied to every string of every result and log line.                                                                                                       |
| `src/safety/errors.ts`        | The closed error vocabulary with fixed messages; maps database and provider failures to a code plus at most a SQLSTATE or a failure kind.                                                                                                                                               |
| `src/bounds.ts`               | Every bound as a fixed constant: page sizes, list lengths, string lengths, result bytes, deadlines, statement timeout, rate limit, concurrency, pool size.                                                                                                                              |
| `src/store/read-store.ts`     | The read-only store interface: the only operations the tools need, with row types that carry no note, actor, raw cell or body text.                                                                                                                                                     |
| `src/store/postgres-store.ts` | The PostgreSQL implementation through `@cas/database`: every method inside a `REPEATABLE READ`, `READ ONLY` transaction with a statement timeout; parameterized SQL; text columns cut to their display bound in the query.                                                              |
...
| `src/engines/live-graph.ts`   | `LiveSignalSource` over the Sprint 1 `GraphGatewayClient`, registry targets only, concurrent per-target queries, identity gate and freshness; no store, replay or fixture reference, enforced by a test.                                                                                |
| `src/tools/*.ts`              | The four tools, each a pure function of the store, the engines and validated arguments.                                                                                                                                                                                                 |
| `src/runtime.ts`              | The invocation path: name check, sliding-window rate limit and concurrency cap, hardened validation, wall-clock deadline, output contract check, redaction, size bound, one redacted single-line log record per call; and `createRuntime`, which reads exactly three environment names. |
| `src/server.ts`               | `McpServer` with the `tools` capability only, fixed instructions, and handlers that return every failure as an `isError` result with a fixed code.                                                                                                                                      |
| `src/bin.ts`                  | The stdio entry point: stderr-only diagnostics, exit on stdin close, SIGINT and SIGTERM after closing the transport and the pool, a fixed line and exit code 2 on a rejected configuration.                                                                                             |

Inputs: `DATABASE_URL` (optional; store-backed tools fail with a fixed code without it),
`GRAPH_API_KEY` (live mode only), `GRAPH_GATEWAY_URL` (optional). Outputs: MCP tool results
with structured content validated against the advertised output schema. Tests: the default
suite over linked in-memory transports and the built entry point as a child process, with a
synthetic provider; the PostgreSQL suite over a seeded schema with a before-and-after digest.
