...
| `@cas/dashboard`      | Next.js application, as Plan 2.0 fixes: command center, review queue, incident explorer, draft editor; judge login. The review workflow here is the only writer of `ReviewState`. Sprint 6.                                          | parallel track (D27, speculative, pending audit): Next.js 16 application with role-gated views, sessions and a server-only data-access layer; account persistence paused                                                                 |
