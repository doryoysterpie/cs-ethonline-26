...
| `@cas/database`       | Postgres schema, migrations and typed access for imported sources, normalized records, classification decisions, the review queue, review records, incidents, evidence and drafts. Sprint 2.                                         | implemented (Sprints 2 to 4): migration runner, six migrations, ingestion, classification and clustering ops                              |
...
| `@cas/clustering/src/engine.ts`        | The three stages, the bounded inverted index, the union-find and the deterministic representative and fingerprint selection. Complexity is bounded per item; it is never quadratic in the corpus.                                                                                                                              |
...
| `@cas/worker/src/clustering/review.ts` | The append-only human layer: merge, split and the deterministic replay that produces the effective view.                                                                                                                                                                                                                       |
