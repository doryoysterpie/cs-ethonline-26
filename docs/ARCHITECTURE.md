...
| `@cas/dashboard`      | Next.js application, as Plan 2.0 fixes: command center, review queue, incident explorer, draft editor; judge login. The review workflow here is the only writer of `ReviewState`. Sprint 6.                                          | production candidate (D28, pending audit): Next.js 16 application with role-gated views, sessions and a server-only data-access layer; PostgreSQL persistence (migration 0010); judge role holds no draft-preview capability             |
...
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- |
| Runtime         | Node 24.21.0 exactly (`.nvmrc`), `engines` on that line; asserted at run time by `tools/checks/toolchain.ts` (security-foundation track, pending audit) | security necessity      |
...
| `src/client.ts`      | `GraphGatewayClient`: `POST {base}/subgraphs/id/{id}`, API key only in `Authorization: Bearer`, explicit timeout, injectable fetch. Distinguishes `credential`, `http`, `graphql`, `schema`, `validation`, `indexing`, `timeout`, `network`, `limit` failures, including failures while reading the body; the body is read under a streaming byte limit (`bounded-body.ts`) and the JSON document under depth and collection limits (`json-shape.ts`), with a fixed cap on requests in flight (`RESOURCE_LIMITS.graph`, security-foundation track, pending audit). |
...
| `src/cli.ts`                  | `db migrate`, `db check`, `editorial validate`, `editorial import`, `editorial report`; a non-empty `DATABASE_URL` is validated in full by `parseDatabaseConfig` before any command is dispatched, validation included; a batch id must be a UUID; the base redactor covers the whole `DATABASE_URL` and its raw and percent-decoded password; every emitted entry passes the redactor and then the single-line guard; exit codes 0, 2 configuration, 3 structural, 4 database, 5 unexpected, 124 command deadline expired, 130 interrupted; SIGINT, SIGTERM and the command deadline (`deadline.ts`, `RESOURCE_LIMITS.command`, security-foundation track, pending audit) abort the import and roll it back. |
...
Implemented and tested in Sprint 5 (`SPRINT-5-REPORT.md`, decisions D25 and D26), corrected
after an independent audit, and **pending re-audit**. Both new packages are pure: no database, no network, no environment, no
...
| `@cas/worker/src/evidence/signals.ts`  | Signal ingestion, two functions with two input types that share no field but a discriminant. A file carries `fixture` or `replay` only and `live` is refused before the path is opened; validated Graph-client evaluations become a `live` run through a separate function that takes no path and no origin argument. The snapshot validator is a closed boundary applied recursively: plain prototype, no symbols, exact own names, data descriptors inspected before any read, no proxy. |
| `@cas/worker/src/evidence/claim.ts`    | Recording a claim about an incident, by a named person: a bounded kind and statement, and the source row it rests on, read from the membership table rather than the request. A claim is a record, not a UUID. Append-only; a repeat is the same claim by fingerprint.                                                                                                                                                                                                                     |
...
| `@cas/worker/src/evidence/review.ts`   | The narrow surface where a suggestion becomes evidence. A `supports` or `conflicts` decision names a recorded claim of the association's incident under the same run, batch and origin, or is refused before anything is hashed. Payload-complete idempotency including actor, rationale and claim identity, and the Sprint 4 note policy applied first.                                                                                                                                   |
...
| `@cas/worker/src/drafting/generate.ts` | Publishes the draft: under one authorised root fixed by the worker, never through a symbolic link, and never overwritten. Identifier and calendar date validated by `@cas/drafting`; both files staged exclusively with mode 0600, flushed, and published together by atomic rename or not at all (D3, provisional).                                                                                                                                                                       |
| `@cas/worker/src/hygiene/scan.ts`      | A root-configurable scanner for prohibited code points in source, decoding by code point; proven against a disposable tree by the repository hygiene test.                                                                                                                                                                                                                                                                                                                                 |
...
Migration 0009 (audit correction, D26) binds an evidence run's origin to its signal run and its
clustering run by composite foreign key, refuses a live signal run served from a reserved-domain
host, and adds `incident_claims`, whose composite foreign key to the membership table proves the
cited source row is a member of the cited incident under the cited run. Associations and states
bind to a claim by foreign key; the `evidence_claim_guard` trigger binds review actions and
checks origin for all three. Existing rows are validated before each constraint and a
contradiction fails the migration atomically; nothing is rewritten.
...
## 14. Security-foundation track: limits, checks and supply chain (pending audit)
...
| -------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/contracts/src/limits.ts`                             | `resource-limits@1`: the measured, versioned ceilings for import, Graph response, draft and command duration, with the measurements they were sized against and the four-times headroom rule; constants only, pinned by test.                                                                                                           |
| `apps/worker/src/editorial/csv-stream.ts`                      | Enforces the import limits while the file streams: file bytes per chunk, column count and cell bytes before a record reaches a handler, row count and retained bytes as rows arrive, the parser's record buffer. A crossed limit is a fixed `structural` refusal with numeric details, before any write.                                |
| `apps/worker/src/deadline.ts`                                  | The command deadline: abort at expiry, exit `124` after a grace period; `CAS_COMMAND_DEADLINE_MS` lowers it and never raises it.                                                                                                                                                                                                        |
| `packages/graph-evidence/src/bounded-body.ts`, `json-shape.ts` | Streaming body read under a byte limit with declared-length distrust; JSON depth bounded before parsing and collections after; the `limit` failure kind.                                                                                                                                                                                |
| `tools/checks/`                                                | First-party, dependency-free scans over every tracked file (`git ls-files`): UTF-8 and invisible-character hygiene, forbidden files and index modes, secret shapes, workflow policy, and the toolchain assertion; the Linux network-denial runner. Each is proven on the clean tree and on a planted defect in a disposable repository. |
| `tools/supply-chain/`                                          | A reader for `pnpm-lock.yaml` version 9 that fails closed, and the generator of the reproducible CycloneDX 1.6 bill of materials and licence inventory, cross-checked against pnpm's own generator.                                                                                                                                     |
| `supply-chain/`                                                | The committed `sbom.cdx.json` and `LICENSES.md`, regenerated on every lockfile change and reproduced byte for byte in CI.                                                                                                                                                                                                               |
| `.github/workflows/ci.yml`, `codeql.yml`                       | The `Verify`, `PostgreSQL integration` and `Supply chain` jobs and the CodeQL analysis; every action SHA-pinned, the service image digest-pinned, read-only permissions.                                                                                                                                                                |
| `docs/THREAT_MODEL.md` and companions                          | The threat model, risk register, data classification and retention, incident-response preparation, disclosure policy, and the track's report.                                                                                                                                                                                           |

Dependency direction is unchanged: `@cas/contracts` gains constants only; the worker and the
Graph client read them; `tools/` depends on nothing in `apps/` or `packages/` and on no
third-party package beyond the root `@types/node` typings.

## 15. Parallel track boundary: `@cas/sheets-intake`
...
## 16. MCP tooling track boundary: `@cas/mcp-server`
...
the advertised output schema. Tests: 208 offline tests in 22 files over linked in-memory
