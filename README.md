...
| `apps/worker`             | `@cas/worker`         | implemented: CSV validation, import, classification, clustering, evidence, anomaly and drafting commands; import limits and a command deadline (security-foundation track, pending audit); 167 offline tests, 95 PostgreSQL |
...
| `packages/contracts`      | `@cas/contracts`      | editorial and import enums, the chain set, the Graph evidence contracts and the versioned resource limits (`resource-limits@1`); 16 tests                                                                                   |
...
| `packages/graph-evidence` | `@cas/graph-evidence` | implemented: live gateway client with streaming body and JSON shape limits, adapter, TVL delta, identity gate, probe; 122 unit tests                                                                                        |
...
| `tools`                   |                       | `offline-sandbox.sb` (macOS network denial), `checks/` (hygiene, forbidden-file, secret, workflow and toolchain scans; Linux network denial), `supply-chain/` (bill of materials generator); 25 tests                       |
...
| `docs/THREAT_MODEL.md`                  | assets, boundaries, roles, entry points, flows, adversaries, abuse cases, mitigations mapped to tests, residual risks (track, pending audit) |
| `docs/RISK_REGISTER.md`                 | risks with owner, status, next action and review date (track, pending audit)                                                                 |
| `docs/DATA_CLASSIFICATION_RETENTION.md` | data classes, inventory, retention and removal (track, pending audit)                                                                        |
| `docs/INCIDENT_RESPONSE.md`             | roles, severities, triggers and runbooks (track, pending audit)                                                                              |
| `docs/VULNERABILITY_DISCLOSURE.md`      | how to report, scope, timelines, safe harbour (track, pending audit)                                                                         |
| `docs/SECURITY-FOUNDATION-REPORT.md`    | the track's report and Codex Desktop handoff: limits, measurements, CI, tests, coverage                                                      |
