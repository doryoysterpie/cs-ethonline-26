 ...
 * values only: basenames, hashes, counts, row numbers, statuses, durations,
 * issue codes, known header names and fixed messages. Unknown header names
 * are reported as a count. Nothing here receives a title, URL, summary,
 * description, raw row or absolute path.
 */
...
export function formatValidation(report: ValidationReport): string[] {
  ...
  const lines = [
    `editorial:validate: structure OK; file=${s.basename}; kind=${report.sourceKind}; bytes=${s.byteLength}; sha256=${s.sha256}; bom=${s.bom ? 'yes' : 'no'}`,
...
function formatReview(review: ImportOutcome['review']): string {
  if (review === null) return 'review snapshot: none (master import)';
  const counts = review.entries.map((e) => `${e.reviewState}=${e.count}`).join(' ');
  return `review snapshot: label=${review.reviewLabel} entries: ${counts.length > 0 ? counts : 'none'}; entriesOnQuarantinedRows=${review.entriesOnQuarantinedRows}`;
}
...
export function formatImportOutcome(outcome: ImportOutcome): string[] {
  ...
  return [
    `${head} status=${b.status} kind=${b.sourceKind} origin=${b.dataOrigin} label=${b.reviewLabel ?? '-'} file=${b.sourceBasename} sha256=${b.fileSha256} bytes=${b.byteLength}`,
...
export function formatBatchReport(report: BatchReport): string[] {
  ...
  return [
    `batch=${b.id} status=${b.status} kind=${b.sourceKind} origin=${b.dataOrigin} label=${b.reviewLabel ?? '-'} file=${b.sourceBasename} sha256=${b.fileSha256} bytes=${b.byteLength} importer=${b.importerVersion}`,
...
/** Fixed message, kind, code and safe details only. The redactor is applied as a last line of defence. */
