    ...
    const incidents = [
      {
        ...
        summary: {
          ...first.summary,
          // The store fetches every bounded column as a prefix plus the stored
          // value's true size; these values are short, so they are fetched whole.
          headline: boundedText(`k ${encodeURIComponent(KEY)} p ${forms(PASSWORD)[4]}`),
        },
        sources: [
          {
            ...first.sources[0]!,
            title: boundedText(`t ${encodeURI(KEY)}`),
            publisher: boundedText(`pub ${encodeURIComponent(PASSWORD).toLowerCase()}`),
            url: boundedText(
              `https://seed.example/x?key=${encodeURIComponent(KEY)}&pw=${PASSWORD}`,
            ),
          },
        ],
      },
