 ...
 * protocol cancellation or the runtime's shutdown. That one signal reaches
 * the store provider, which opens the call's single transaction with it,
 * checks it before every statement and cancels a statement in flight at the
 * server, and the live source, which aborts its request sockets. When it
 * fires, the runtime waits for the work to unwind, bounded by a grace period,
 * before it reports; the concurrency permit is released only once the work
 * has actually unwound, that is, once the transaction has rolled back and its
 * connection has been destroyed or the fetch has been abandoned; and shutdown
 * aborts every active call and waits, bounded, for the same.
 *
...
export function createRuntime(options: RuntimeOptions): ToolRuntime {
  ...
  const close = async (): Promise<void> => {
    ...
    if (store !== null) {
      // A call whose connection is stuck would keep the process alive; the wait is bounded.
 ...
 * reported; whether or not it unwound in time is recorded in the log line.
 */
...
export async function invokeTool(
  ...
  try {
    ...
    const startedWork = dispatch(runtime, tool, validated, {
      signal: callScope.signal,
      redact: runtime.redact,
    });
    work = startedWork;
    const unwound = startedWork.then(
      () => undefined,
      () => undefined,
    );
    void unwound.then(() => {
    ...
    const produced = await awaitWithinScope(startedWork, callScope, runtime.unwindGraceMs);
