...
message and content-free details. Codes are a closed vocabulary of twenty-two
(`invalid_arguments`, `evidence_run_not_found`, `evidence_run_not_completed`,
`incident_not_found`, `signal_run_not_found`, `signal_run_not_completed`,
`stored_metadata_invalid`, `database_not_configured`, `database_configuration_invalid`,
`database_role_overprivileged`, `database_unavailable`, `database_query_failed`,
`graph_credential_missing`, `graph_gateway_invalid`, `graph_provider_failed`,
`tool_timeout`, `call_cancelled`, `result_too_large`, `rate_limited`,
...
environment value. Every error the server can produce is one of these: the `tools/call`
handler is installed at the request-handler level, so an unknown name or an unexpected
argument key is answered with `unknown_tool` or `invalid_arguments` rather than with the
...
call's transaction is rolled back, a statement still running is cancelled at the server, a
